...
public DoacaoResponseDTO cadastrar(DoacaoDTO doacaoDTO) {
    ...
    Doador doador = doadorRepository.findById(doacaoDTO.getCliente())
            .orElseThrow(() -> new RestNotFoundException("Doador não encontrado com o id: " + doacaoDTO.getCliente()));
    ...
    if (doacaoDTO.getSolicitacaoProduto() != null) {
        ...
        // Verificar se o cliente já fez uma doação anteriormente para a mesma solicitação de SolicitacaoProduto
        boolean hasExistingDoacao = doacaoRepository.existsByDoadorAndSolicitacaoProduto(doador, solicitacaoProduto);
    ...
    public DoacaoResponseDTO atualizar(Long id, DoacaoDTO doacaoDTO) {
        ...
        if (doacaoDTO.getCliente() != null) {
            Doador doador = doadorRepository.findById(doacaoDTO.getCliente())
                    .orElseThrow(() -> new RestNotFoundException(
                            "Doador não encontrado com o id: " + doacaoDTO.getCliente()));
            doacao.setDoador(doador);
        }
        ...
        if (doacaoDTO.getSolicitacaoProduto() != null) {
            SolicitacaoProduto solicitacaoProduto = solicitacaoProdutoRepository
                    ...
                    .orElseThrow(() -> new RestNotFoundException(
                            "Solicitação de Produto não encontrada com o id: "
